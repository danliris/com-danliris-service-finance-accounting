...
namespace Com.Danliris.Service.Finance.Accounting.Lib.BusinessLogic.Services.VBVerification
{
    public class VbVerificationService : IVbVerificationService
    {
        ...
        {
            var query = _dbContext.RealizationVbs.Where(a => a.Position == (int)VBRealizationPosition.Purchasing).AsQueryable();
        ...
        {
            var query = _dbContext.RealizationVbs.Where(a => a.Position == (int)VBRealizationPosition.PurchasingToVerification).AsQueryable();
            var query2 = _RequestDbSet.AsQueryable();
